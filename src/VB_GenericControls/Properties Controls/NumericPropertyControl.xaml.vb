...
Public Class NumericPropertyControl
...
#Region "Members"
    ...
    ''' </summary>
    Public Shared NumberProperty As DependencyProperty = DependencyProperty.Register(NameOf(Number), GetType(Double), GetType(NumericPropertyControl), New UIPropertyMetadata(CDbl(0), AddressOf NumberChangedCallback))
    ...
    Private Shared Sub NumberChangedCallback(d As DependencyObject, e As DependencyPropertyChangedEventArgs)
        ...
        If d.GetType <> GetType(NumericPropertyControl) Then Exit Sub
        Dim thisControl = DirectCast(d, NumericPropertyControl)
        '
        ...
        End If
        '
        If thisControl.CanHaveNaN = False AndAlso Double.IsNaN(newNumber) Then
            thisControl.ValueIsValid = False
        ...
        'End If
        '
        If thisControl.IncludeBounds Then
            If (thisControl.Number <= thisControl.MinValue) OrElse (thisControl.Number >= thisControl.MaxValue) Then
                thisControl.ValueIsValid = False
                thisControl.ToolTip = $"Number must be greater than '{thisControl.MinValue}' and less than '{thisControl.MaxValue}'."
                Exit Sub
            End If
        Else
            If (thisControl.Number < thisControl.MinValue) OrElse (thisControl.Number > thisControl.MaxValue) Then
                thisControl.ValueIsValid = False
                thisControl.ToolTip = $"Number must be greater than or equal to '{thisControl.MinValue}' and less than or equal to '{thisControl.MaxValue}'."
                Exit Sub
            End If
        End If
        '
    ...
    ''' </summary>
    Public Shared CanHaveNegativeProperty As DependencyProperty = DependencyProperty.Register(NameOf(CanHaveNegative), GetType(Boolean), GetType(NumericPropertyControl), New PropertyMetadata(True))
    ...
    ''' Dependency property for the can have negative property. 
    ''' </summary>
    Public Shared IncludeBoundsProperty As DependencyProperty = DependencyProperty.Register(NameOf(IncludeBounds), GetType(Boolean), GetType(NumericPropertyControl), New PropertyMetadata(False))

    ''' <summary>
    ''' Determines if the number can be negative. 
    ''' </summary>
    Public Property IncludeBounds As Boolean
        Get
            Return DirectCast(GetValue(IncludeBoundsProperty), Boolean)
        End Get
        Set(value As Boolean)
            SetValue(IncludeBoundsProperty, value)
        End Set
    End Property
    ...
    ''' Dependency property for the can have negative property. 
    ''' </summary>
    Public Shared CanHaveNaNProperty As DependencyProperty = DependencyProperty.Register(NameOf(CanHaveNaN), GetType(Boolean), GetType(NumericPropertyControl), New PropertyMetadata(True))

    ''' <summary>
    ''' Determines if the number can be negative. 
    ''' </summary>
    Public Property CanHaveNaN As Boolean
        Get
            Return DirectCast(GetValue(CanHaveNaNProperty), Boolean)
        End Get
        Set(value As Boolean)
            SetValue(CanHaveNaNProperty, value)
        End Set
    End Property
    ...
    ''' </summary>
    Public Shared IsTextBoxEnabledProperty As DependencyProperty = DependencyProperty.Register(NameOf(IsTextBoxEnabled), GetType(Boolean), GetType(NumericPropertyControl), New UIPropertyMetadata(True))
    ...
    ''' </summary>
    Public Shared IsWholeNumberProperty As DependencyProperty = DependencyProperty.Register(NameOf(IsWholeNumber), GetType(Boolean), GetType(NumericPropertyControl), New PropertyMetadata(False))
    ...
    ''' </summary>
    Public Shared MaxValueProperty As DependencyProperty = DependencyProperty.Register(NameOf(MaxValue), GetType(Double), GetType(NumericPropertyControl), New FrameworkPropertyMetadata(Double.MaxValue, AddressOf MinMaxChangedCallback))
    ''' <summary>
    ''' Property Changed Callback for the Number property.
    ''' </summary>
    Private Shared Sub MinMaxChangedCallback(d As DependencyObject, e As DependencyPropertyChangedEventArgs)
        If IsNothing(d) Then Exit Sub
        If d.GetType <> GetType(NumericPropertyControl) Then Exit Sub
        Dim thisControl = DirectCast(d, NumericPropertyControl)
        '
        If thisControl.CanHaveNaN = False AndAlso Double.IsNaN(thisControl.Number) Then
            thisControl.ValueIsValid = False
            thisControl.ToolTip = $"NaN is not a number."
            Exit Sub
        End If
    ...
    ''' </summary>
    Public Shared MinValueProperty As DependencyProperty = DependencyProperty.Register(NameOf(MinValue), GetType(Double), GetType(NumericPropertyControl), New FrameworkPropertyMetadata(Double.MinValue, AddressOf MinMaxChangedCallback))
    ...
    ''' </summary>
    Public Shared TitleProperty As DependencyProperty = DependencyProperty.Register(NameOf(Title), GetType(String), GetType(NumericPropertyControl), New UIPropertyMetadata("Title"))
...
#Region "Control Width"
    ...
    ''' </summary>
    Public Shared MaxPropertyWidthProperty As DependencyProperty = DependencyProperty.Register(NameOf(MaxPropertyWidth), GetType(Double), GetType(NumericPropertyControl), New UIPropertyMetadata(DefaultMaxPropertyWidth))
    ...
    ''' </summary>
    Public Shared MinPropertyWidthProperty As DependencyProperty = DependencyProperty.Register(NameOf(MinPropertyWidth), GetType(Double), GetType(NumericPropertyControl), New UIPropertyMetadata(DefaultMinPropertyWidth))
    ...
    ''' </summary>
    Public Shared PropertyWidthProperty As DependencyProperty = DependencyProperty.Register(NameOf(PropertyWidth), GetType(GridLength), GetType(NumericPropertyControl), New UIPropertyMetadata(DefaultPropertyWidth))
    ...
    ''' </summary>
    Public Shared ShowLeaderLineProperty As DependencyProperty = DependencyProperty.Register(NameOf(ShowLeaderLine), GetType(Boolean), GetType(NumericPropertyControl), New UIPropertyMetadata(True))
