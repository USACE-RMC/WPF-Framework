...
Public Module GeneralMethods
    ...
    ''' <returns></returns>
    Public Function FindElementByName(Of T As FrameworkElement)(ByVal element As FrameworkElement, ByVal childName As String) As T
        Dim childElement As T = Nothing
        Dim childCount = VisualTreeHelper.GetChildrenCount(element)

        If childCount = 0 AndAlso element.GetType = GetType(Primitives.Popup) Then
            Dim popupElement = DirectCast(element, Primitives.Popup)
            If Not IsNothing(popupElement.Child) Then Return FindElementByName(Of T)(CType(popupElement.Child, FrameworkElement), childName)
        End If

        For i As Integer = 0 To childCount - 1
            Dim child As FrameworkElement = TryCast(VisualTreeHelper.GetChild(element, i), FrameworkElement)
            If child Is Nothing Then Continue For

            If TypeOf child Is T AndAlso child.Name.Equals(childName) Then
                childElement = CType(child, T)
                Exit For
            End If

            childElement = FindElementByName(Of T)(child, childName)
            If childElement IsNot Nothing Then Exit For
        Next
    ...
    ''' <returns></returns>
    Public Function GetAllElementsOfType(element As Visual, t As Type) As List(Of Visual)
        Dim r As New List(Of Visual)
        For i As Int32 = 0 To VisualTreeHelper.GetChildrenCount(element) - 1
            Dim v As Visual = CType(VisualTreeHelper.GetChild(element, i), Visual)
            If IsNothing(v) Then Continue For
            r.AddRange(GetAllElementsOfType(v, t))
            If v.GetType = t Then r.Add(v)
        Next
        Return r
    End Function
    ...
    Public Function HasWebConnection() As Boolean
        Try
    ...
    Public Function FileOpenDialog(filters As String) As String
        Dim OpenfileDialog As New OpenFileDialog With {.Filter = filters}
    ...
    ''' <returns></returns>
    Public Function FileSaveDialog(filters As String, Optional overwritePrompt As Boolean = True) As String
        Dim SaveFileBrowser As New SaveFileDialog With {.overwritePrompt = overwritePrompt, .Filter = filters}
    ...
    Public Function FolderBrowserDialog(Optional owner As Window = Nothing, Optional title As String = Nothing, Optional initialDirectory As String = Nothing, Optional defaultDirectory As String = Nothing) As String
        Dim FolderBrowser As New FolderBrowser With {.title = title, .initialDirectory = initialDirectory, .defaultDirectory = defaultDirectory}
        If FolderBrowser.ShowDialog(owner) = True Then
    ...
    Public Sub CopyFile(sourceFilePath As String, destinationFilePath As String, Optional progress As IProgress(Of Int32) = Nothing)
        Dim buffer As Byte() = New Byte(1024 * 1024 - 1) {} ' 1MB buffer
        Using source As New FileStream(sourceFilePath, FileMode.Open, FileAccess.Read)
            Dim fileLength As Long = source.Length
            Using dest As New FileStream(destinationFilePath, FileMode.CreateNew, FileAccess.Write)
                Dim totalBytes As Long = 0
                Dim currentBlockSize As Integer = source.Read(buffer, 0, buffer.Length)
                While (currentBlockSize > 0)
                    totalBytes += currentBlockSize
                    dest.Write(buffer, 0, currentBlockSize)
                    'need to write an action bit here.
                    If IsNothing(progress) Then progress.Report(CInt(totalBytes * 100.0 / fileLength))
    ...
    Public Sub CopyDirectory(sourcePath As String, destPath As String)
        ...
        Next
    End Sub
    Public Sub CopyDirectory(sourcePath As String, destPath As String, theProgressBar As ProgressBar, progressText As TextBlock)
        If Not Directory.Exists(destPath) Then
            Directory.CreateDirectory(destPath)
        End If
        ...
        Dim DirectoryFiles() As String = Directory.GetFiles(sourcePath)
        Dim updatePbDelegate As New UpdateProgressBarDelegate(AddressOf theProgressBar.SetValue)
        Dim UpdatePbTDelegate As New UpdateProgressBarDelegate(AddressOf progressText.SetValue)
        For i As Int32 = 0 To DirectoryFiles.Count - 1
            Dim dest As String = Path.Combine(destPath, Path.GetFileName(DirectoryFiles(i)))
            File.Copy(DirectoryFiles(i), dest)
            Dispatcher.CurrentDispatcher.Invoke(updatePbDelegate, DispatcherPriority.Background, New Object() {ProgressBar.ValueProperty, CDbl(100 * i / DirectoryFiles.Count)})
            Dispatcher.CurrentDispatcher.Invoke(UpdatePbTDelegate, DispatcherPriority.Background, New Object() {TextBlock.TextProperty, CInt(100 * CDbl(i / DirectoryFiles.Count)) & "% Copying From " & DirName})
        Next
        '
        For Each folder As String In Directory.GetDirectories(sourcePath)
            Dim dest As String = Path.Combine(destPath, Path.GetFileName(folder))
        ...
        Dispatcher.CurrentDispatcher.Invoke(updatePbDelegate, DispatcherPriority.Background, New Object() {ProgressBar.ValueProperty, CDbl(0)})
    ...
    Public Function DeleteDirectory(dir As String) As Boolean
        Try
    ...
    ''' </remarks>
    Public Function IsNumericType(typeToTest As Type) As Boolean
    ...
    ''' </summary>
    Public Function Bitmap2BitmapSource(bitmap As System.Drawing.Bitmap) As BitmapSource
