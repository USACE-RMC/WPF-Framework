...
namespace OxyPlotControls.Demo
{
    ...
    public partial class MainWindow : MetroWindow
    {
        ...
        private void MainWindow_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            // Unsubscribe when the window closes to prevent memory leaks. As of the
            // OxyPlotToolbar refactor, cursor resources live in a process-wide static
            // cache so the toolbar no longer implements IDisposable.
            Closing += (s, e) =>
            {
                ThemeService.Instance.ThemeChanged -= OnThemeChanged;
            };
        ...
        private void LoadSettingsButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (File.Exists(fileToOpen))
            {
                ...
                if (elements.Count > 0 && elements[0]?.OuterXml != null)
                {
                    OxyPlotControls.OxyPlotSettingsSerializer.FromXelement(TestPlot, XElement.Parse(elements[0]!.OuterXml));
                }
        ...
        /// <summary>
        /// Handles the theme radio button checked event to switch application themes.
        /// </summary>
        ...
        /// </summary>
        /// <summary>
        /// Creates a 100K-point line series for performance testing.
        /// </summary>
        private void LargeLineSeries_Create(bool logarithmic)
        {
            TestPlot.Series.Clear();
            TestPlot.Axes.Clear();
            TestPlot.Annotations.Clear();
            ...
            TestPlot.ActualModel.Annotations.Clear();
            _demoDataRegistry.Clear();

            string axisType = logarithmic ? "Log" : "Linear";
            TestPlot.Title = $"100K Line Series ({axisType})";

            if (logarithmic)
            {
                TestPlot.Axes.Add(new OxyPlot.Wpf.LogarithmicAxis
                {
            ...
            {
                TestPlot.Axes.Add(new OxyPlot.Wpf.LinearAxis
                {
                    Position = AxisPosition.Left,
                    Title = "Value",
                    Key = "y"
            ...
            {
                Title = $"100K Random Data ({axisType})",
            ...
            {
                double x = logarithmic ? i + 1.0 : i;
                double y = logarithmic ? Math.Abs(rng.NextDouble() * 100) + 0.01 : rng.NextDouble() * 100;
                internalSeries.Points.Add(new DataPoint(x, y));
            }
        ...
        /// <summary>
        /// Creates a 100K-point scatter series for performance testing.
        /// </summary>
        private void LargeScatterSeries_Create(bool logarithmic)
        {
            TestPlot.Series.Clear();
            TestPlot.Axes.Clear();
            TestPlot.Annotations.Clear();
            ...
            string axisType = logarithmic ? "Log" : "Linear";
            TestPlot.Title = $"100K Scatter Series ({axisType})";
            ...
            var scatterSeries = new OxyPlot.Wpf.ScatterSeries
            {
                Title = $"100K Random Scatter ({axisType})",
            ...
            for (int i = 0; i < 100_000; i++)
            {
                double x = logarithmic ? rng.NextDouble() * 999 + 1.0 : rng.NextDouble() * 1000;
                double y = logarithmic ? rng.NextDouble() * 999 + 1.0 : rng.NextDouble() * 1000;
                internalSeries.Points.Add(new OxyPlot.Series.ScatterPoint(x, y));
            }
        ...
        private void ReRenderButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                RenderTimeLabel.Text = "Load data first by selecting a series type";
                return;
            }
        ...
        private void ComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            switch (selectedItem.Content.ToString())
            {
                ...
                case "Two Color Line Series":
                    TwoColorLineSeries_Create();
                    break;
                case "Three Color Line Series":
                    ThreeColorLineSeries_Create();
                    break;
                case "Pie Series":
                    PieSeries_Create();
                    break;
                case "Candlestick Series":
                    CandleStickSeries_Create();
                    break;
                case "Stair Step Series":
                    StairStepSeries_Create();
                    break;
                case "Stem Series":
                    StemSeries_Create();
                    break;
                case "100K Line Series (Linear)":
                    LargeLineSeries_Create(logarithmic: false);
                    break;
                case "100K Line Series (Log)":
                    LargeLineSeries_Create(logarithmic: true);
                    break;
                case "100K Scatter Series (Linear)":
                    LargeScatterSeries_Create(logarithmic: false);
                    break;
                case "100K Scatter Series (Log)":
                    LargeScatterSeries_Create(logarithmic: true);
                    break;
            }
        ...
        private class DummyMultiPurposePoint
        {
            ...
            public string LabelVal { get; set; } = string.Empty;
        ...
        /// <summary>
        /// Creates a two-color line series demo.
        /// </summary>
        private void TwoColorLineSeries_Create()
        {
            TestPlot.Series.Clear();
            TestPlot.Axes.Clear();

            TestPlot.Title = "Two Color Line Series";
            ...
            var series = new OxyPlot.Wpf.TwoColorLineSeries
            {
                Title = "Two Color Line",
                Color = Colors.Blue,
                Color2 = Colors.Red,
                Limit = 0.5,
                StrokeThickness = 2
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                ((OxyPlot.Series.TwoColorLineSeries)series.InternalSeries).Points.Add(new DataPoint(x, y));
            }

            RegisterDemoData(series.Title, points);

            TestPlot.Series.Add(series);
            TestPlot.ResetAllAxes();
            TestPlot.InvalidatePlot(true);
        }

        /// <summary>
        /// Creates a three-color line series demo.
        /// </summary>
        private void ThreeColorLineSeries_Create()
        {
            TestPlot.Series.Clear();
            TestPlot.Axes.Clear();

            TestPlot.Title = "Three Color Line Series";
            ...
            var series = new OxyPlot.Wpf.ThreeColorLineSeries
            {
                Title = "Three Color Line",
                Color = Colors.Green,      // Middle color (between limits)
                ColorHi = Colors.Red,      // Above high limit
                ColorLo = Colors.Blue,     // Below low limit
                LimitHi = 0.5,
                LimitLo = -0.5,
                StrokeThickness = 2
            };
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                double y = Math.Sin(x);
                points.Add(new DataPoint(x, y));
                ((OxyPlot.Series.ThreeColorLineSeries)series.InternalSeries).Points.Add(new DataPoint(x, y));
            }
        ...
        /// <summary>
        /// Creates a pie series demo.
        /// </summary>
        private void PieSeries_Create()
        {
            TestPlot.Series.Clear();
            TestPlot.Axes.Clear();

            TestPlot.Title = "Pie Series";

            var pieSeries = new OxyPlot.Wpf.PieSeries
            {
                Title = "Pie Chart",
                StrokeThickness = 2,
                Stroke = Colors.White,
            ...
            {
                new PieSlice("Category A", 30) { Fill = OxyColors.Red },
                new PieSlice("Category B", 25) { Fill = OxyColors.Blue },
                new PieSlice("Category C", 20) { Fill = OxyColors.Green },
                new PieSlice("Category D", 15) { Fill = OxyColors.Orange },
                new PieSlice("Category E", 10) { Fill = OxyColors.Purple }
            };

            var internalPieSeries = (OxyPlot.Series.PieSeries)pieSeries.InternalSeries;
            foreach (var slice in slices)
            {
                internalPieSeries.Slices.Add(slice);
            }

            RegisterDemoData(pieSeries.Title, slices);

            TestPlot.Series.Add(pieSeries);
            TestPlot.ResetAllAxes();
            TestPlot.InvalidatePlot(true);
        }

        /// <summary>
        /// Creates a candlestick series demo.
        /// </summary>
        private void CandleStickSeries_Create()
        {
            TestPlot.Series.Clear();
            TestPlot.Axes.Clear();

            TestPlot.Title = "Candlestick Series";
            ...
            var series = new OxyPlot.Wpf.CandleStickSeries
            {
                Title = "Stock Price",
            ...
            var items = new List<HighLowItem>();
            double price = 100;
        ...
        /// <summary>
        /// Creates a stair-step series demo.
        /// </summary>
        private void StairStepSeries_Create()
        {
            TestPlot.Series.Clear();
            TestPlot.Axes.Clear();

            TestPlot.Title = "Stair Step Series";
            ...
            var series = new OxyPlot.Wpf.StairStepSeries
            {
            ...
            for (int i = 0; i < 20; i++)
            {
                ...
                var point = new DataPoint(i, value);
                points.Add(point);
                ((OxyPlot.Series.StairStepSeries)series.InternalSeries).Points.Add(point);
            }
        ...
        /// <summary>
        /// Creates a stem series demo.
        /// </summary>
        private void StemSeries_Create()
        {
            TestPlot.Series.Clear();
            TestPlot.Axes.Clear();

            TestPlot.Title = "Stem Series";
            ...
            var series = new OxyPlot.Wpf.StemSeries
            {
                Title = "Stem Data",
                Color = Colors.Blue,
                StrokeThickness = 2,
            ...
            for (int i = 0; i < 50; i++)
            {
                ...
                points.Add(point);
                ((OxyPlot.Series.StemSeries)series.InternalSeries).Points.Add(point);
            }
