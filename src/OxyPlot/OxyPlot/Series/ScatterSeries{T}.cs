...
namespace OxyPlot.Series
{
    ...
    public abstract class ScatterSeries<T> : XYAxisSeries where T : ScatterPoint
    {
        ...
        /// <summary>
        /// Reusable buffer for all screen-space point positions during rendering.
        /// </summary>
        private readonly List<ScreenPoint> allPointsBuffer = new List<ScreenPoint>();

        /// <summary>
        /// Reusable buffer for all marker sizes during rendering.
        /// </summary>
        private readonly List<double> allMarkerSizesBuffer = new List<double>();

        /// <summary>
        /// Reusable buffer for selected screen-space point positions during rendering.
        /// </summary>
        private readonly List<ScreenPoint> selectedPointsBuffer = new List<ScreenPoint>();

        /// <summary>
        /// Reusable buffer for selected marker sizes during rendering.
        /// </summary>
        private readonly List<double> selectedMarkerSizesBuffer = new List<double>();
        ...
        public override void Render(IRenderContext rc)
        {
            ...
            this.allPointsBuffer.Clear();
            this.allMarkerSizesBuffer.Clear();
            this.selectedPointsBuffer.Clear();
            this.selectedMarkerSizesBuffer.Clear();
            var allPoints = this.allPointsBuffer;
            var allMarkerSizes = this.allMarkerSizesBuffer;
            var selectedPoints = this.selectedPointsBuffer;
            var selectedMarkerSizes = this.selectedMarkerSizesBuffer;
            ...
            // For interactive screen rendering with large point counts and no explicit BinSize,
            // auto-compute a bin size to prevent overdraw at the same pixel.
            int effectiveBinSize = this.BinSize;
            if (effectiveBinSize == 0 && rc.RendersToScreen && n > 1000)
            {
                effectiveBinSize = Math.Max(1, (int)(this.MarkerSize * 0.5));
            }
