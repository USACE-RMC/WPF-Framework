...
namespace OxyPlot.Series
{
    ...
    public class ContourSeries : XYAxisSeries
    {
        ...
        public override void Render(IRenderContext rc)
        {
            ...
            if (this.contours == null || this.contours.Count == 0)
            {
                return;
            }
            ...
            foreach (var contour in this.contours)
            {
                ...
                if (transformedPoints.Length <= 10 ||
                    (!double.IsNaN(this.ContourLevelStep) && this.ContourLevelStep > 0 &&
        ...
        {
            // Labels are opt-in: skip when caller has not supplied a LabelFormatString.
            // This prevents phantom label backgrounds from rendering on unlabeled contours.
            if (pts.Length < 2 || string.IsNullOrEmpty(this.LabelFormatString))
            {
                return;
            }
