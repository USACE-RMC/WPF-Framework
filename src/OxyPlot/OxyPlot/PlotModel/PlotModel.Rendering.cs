...
namespace OxyPlot
{
    ...
    public partial class PlotModel
    {
        ...
        private bool AdjustPlotMargins(IRenderContext rc)
        {
            ...
            {
                var requiredSize = this.AdjustAxesPositions(this.Axes, position);
        ...
        /// <summary>
        /// Adjust the positions of parallel axes at a given position, returns total size.
        /// </summary>
        /// <param name="allAxes">All axes (filtered internally by position and visibility).</param>
        /// <param name="position">The axis position to process.</param>
        /// <returns>The maximum value of the position tier.</returns>
        private double AdjustAxesPositions(ElementCollection<Axis> allAxes, AxisPosition position)
        {
            ...
            // Collect distinct tiers for this position (typically 1-2 tiers)
            // Use a simple sorted approach to avoid LINQ GroupBy/OrderBy allocations
            var tierMax = -1;
            for (int i = 0; i < allAxes.Count; i++)
            {
                var a = allAxes[i];
                if (a.IsAxisVisible && a.Position == position && a.PositionTier > tierMax)
                {
                    tierMax = a.PositionTier;
                }
            }

            for (int tier = 0; tier <= tierMax; tier++)
            {
                var maxSizeOfPositionTier = 0.0;
                var hasTier = false;

                for (int i = 0; i < allAxes.Count; i++)
                {
                    var a = allAxes[i];
                    if (a.IsAxisVisible && a.Position == position && a.PositionTier == tier)
                    {
                        hasTier = true;
                        var size = GetSize(a);
                        if (size > maxSizeOfPositionTier)
                        {
                            maxSizeOfPositionTier = size;
                        }
                    }
                }
                ...
                for (int i = 0; i < allAxes.Count; i++)
                {
                    ...
                    if (a.IsAxisVisible && a.Position == position && a.PositionTier == tier)
                    {
                        a.PositionTierSize = maxSizeOfPositionTier;
                        a.PositionTierMinShift = minValueOfPositionTier;
                        a.PositionTierMaxShift = maxValueOfPositionTier;
                    }
                }
            }
        ...
        private void RenderAnnotations(IRenderContext rc, AnnotationLayer layer)
        {
            ...
            foreach (var a in this.Annotations)
            {
                ...
                var currentClippingRect = a.GetClippingRect();
        ...
        private void RenderBackgrounds(IRenderContext rc)
        {
            ...
            {
                if (s.IsVisible && s is XYAxisSeries xys && s.Background.IsVisible())
                {
                    rc.DrawRectangle(xys.GetScreenRectangle(), xys.Background, OxyColors.Undefined, 0, this.EdgeRenderingMode);
                }
            }
        }
        ...
        private void RenderSeries(IRenderContext rc)
        {
            ...
            foreach (var s in this.Series)
            {
                ...
                var currentClippingRect = s.GetClippingRect();
                if (!currentClippingRect.Equals(previousClippingRect))
                {
                    if (!previousClippingRect.Equals(OxyRect.Everything))
                    {
