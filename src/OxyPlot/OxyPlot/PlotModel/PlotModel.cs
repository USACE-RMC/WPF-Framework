...
namespace OxyPlot
{
    ...
    public partial class PlotModel : Model, IPlotModel
    {
        ...
        /// </summary>
        /// <value>The legend area rectangle.</value>
        /// <remarks>
        /// This property is provided for backward compatibility. It returns the area of the first legend if present.
        /// </remarks>
        public OxyRect LegendArea
        {
            get
            {
                var legend = this.Legends.FirstOrDefault();
                return legend?.LegendArea ?? default(OxyRect);
            }
        }
        ...
        private void UpdateBarSeriesManagers()
        {
            ...
            var barSeriesGroups = this.Series
                .Where(s => s.IsVisible)
                .OfType<IBarSeries>()
                .Where(s => s.CategoryAxis != null && s.ValueAxis != null)
