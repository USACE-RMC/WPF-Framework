...
namespace OxyPlot.Axes
{
    ...
    public class LogarithmicAxis : Axis
    {
        ...
        public override void GetTickValues(out IList<double> majorLabelValues, out IList<double> majorTickValues, out IList<double> minorTickValues)
        {
            ...
            else if (ticksPerDecade < (this.PowerPadding ? this.Base : 2))
        ...
        {
            // Clamp non-positive values to a very small positive number rather than
            // returning -1 sentinel. The -1 sentinel maps to a screen coordinate ABOVE
            // the plot area, which causes AreaSeries polygons with baselines at Y=0 to
            // render inverted (at the top instead of bottom). Clamping to 1e-16 maps to
            // a screen coordinate far BELOW the visible axis range, so area fills clip
            // correctly to the bottom of the chart. Consistent with PreTransform behavior.
            if (x <= 0)
            {
                x = 1e-16;
            }
        ...
        internal override void UpdateActualMaxMin()
        {
            ...
            if (this.PowerPadding)
            {
                ...
                // Only power-pad the minimum if NOT user-set AND NOT zoomed
                if (double.IsNaN(this.ViewMinimum) && double.IsNaN(this.Minimum)
                    && !double.IsNaN(this.ActualMinimum) && this.ActualMinimum > 0)
                {
                    var e0 = (int)Math.Floor(Math.Log(this.ActualMinimum) / logBase);
                    this.ActualMinimum = Math.Round(Math.Exp(e0 * logBase), 14);
                }

                // Only power-pad the maximum if NOT user-set AND NOT zoomed
                if (double.IsNaN(this.ViewMaximum) && double.IsNaN(this.Maximum)
                    && !double.IsNaN(this.ActualMaximum) && this.ActualMaximum > 0)
                {
        ...
        {
            // Clamp non-positive values to a very small positive number (Excel-like behavior).
            // This allows series with zero/negative data to render on log axes
            // without breaking. The data point renders below the visible axis range.
            if (x <= 0)
            {
                x = 1e-16;
            }

            return Math.Log(x, this.Base);
        }
